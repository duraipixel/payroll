<form action="" class="" id="dynamic_form">
    ...
    <div class="fv-row form-group mb-10">
        ...
        <div >
            <input type="text" class="form-control" name="duty_type_name" value="{{  $info->name ?? '' }}" id="duty_type_name" required >
        </div>
    </div>
            ...
            form,
            {
                fields: {
                    'duty_type_name': {
						validators: {
							notEmpty: {
								message: 'Duty Type Name is required'
							},
						}
					},
                },
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                if( res.error == 1 ) {
                                    ...
                                    toastr.success("Duty Types added successfully");
                                    $('#kt_dynamic_app').modal('hide');
                                    if (from) {
                                            dtTable.draw();
                                        } else {
                                    if( res.inserted_data ) {
                                        $('#duty_type').append(`<option value="${res.inserted_data.id}">${res.inserted_data.name}</option>`)
                                        $('#duty_type').val(res.inserted_data.id);
                                    }
                                }
                                }
                            }
