<form action="" class="" id="dynamic_form">
    ...
    <div class="fv-row form-group mb-10">
        ...
        <div >
            <input type="text" class="form-control" name="school_name" id="school_name" value="{{  $info->name ?? '' }}" required >
        </div>
    </div>
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                if( res.error == 1 ) {
                                    ...
                                    toastr.success("Schools added successfully");
                                    $('#kt_dynamic_app').modal('hide');
                                    if (from) {
                                            dtTable.draw();
                                        } else {
                                    if( res.inserted_data ) {
                                        $('#other_school').append(`<option value="${res.inserted_data.id}">${res.inserted_data.name}</option>`)
                                        $('#other_school').val(res.inserted_data.id);
                                    }
                                }
                                }
                            }
