<form action="" class="" id="dynamic_form">
   ...
   <input type="hidden" name="form_type" id="form_type" value="{{ $from ?? '' }}">
    ...
    <div class="fv-row form-group mb-10">
        ...
        <div >
            <input type="text" class="form-control" name="place" id="place" value="{{ $info->name ?? '' }}" required >
        </div>
    </div>
                                ...
                                if( res.error == 1 ) {
                                    ...
                                    if (from) {
                                        ...
                                        if( res.inserted_data ) {
                                            $('#place_of_birth_id').append(`<option value="${res.inserted_data.id}">${res.inserted_data.name}</option>`)
                                            $('#place_of_birth_id').val(res.inserted_data.id).trigger('change');
                                        }
                                    }
                                }
                            }
