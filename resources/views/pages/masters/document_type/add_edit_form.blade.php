<form action="" class="" id="dynamic_form">
    ...
    <div class="fv-row form-group mb-10">
        ...
        <div >
            <input type="text" class="form-control" name="document_name" id="document_name" value="{{  $info->name ?? '' }}" required >
        </div>
    </div>
...
<script>
    var from = '{{ $from ?? '' }}';
var KTAppEcommerceSaveDocumentType = function () {
                            ...
                            success: function(res) {
                                ...
                                if( res.error == 1 ) {
                                    ...
                                    if( res.inserted_data ) {
                                        $('#board_id').append(`<option value="${res.inserted_data.id}">${res.inserted_data.name}</option>`)
                                        $('#board_id').val(res.inserted_data.id).trigger('change');
                                    }
                                }
                                }
                            }
...
KTUtil.onDOMContentLoaded(function () {
    KTAppEcommerceSaveDocumentType.init();
});
