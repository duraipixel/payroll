<form class="" id="dynamic_form">
    @csrf
    {{-- <input type="hidden" name="revision[]" value="<?php print_r($transfer ?? ''); ?>"> --}}
    <input type="hidden" name="status" value="{{ $status ?? '' }}">
    <input type="hidden" name="transfer_status" value="{{ $transfer_status ?? '' }}">
                ...
                if (validator) {
                    validator.validate().then(function(status) {
                        ...
                        if (status == 'Valid') {
                            ...
                            var list = $("input[name='transfer[]']:checked").map(function() {
                                return this.value;
                            }).get();
                            var formData = new FormData(forms);
                            formData.append('transfer', list);
                            $.ajax({
                                url: "{{ route('staff.transfer.status') }}",
                                type: "POST",
                                data: formData,
                                processData: false,
                                contentType: false,
