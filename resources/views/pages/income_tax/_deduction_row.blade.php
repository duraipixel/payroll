    ...
    @if (isset($deductions) && !empty($deductions) && count($deductions) > 0)
        @foreach ($deductions as $ded)
            <tr class="delete_row">
                        ...
                        <select name="items[]" id="" class="form-control table-select" required>
                            ...
                            @if (isset($items) && !empty($items))
                                @foreach ($items as $row)
                                    <option value="{{ $row->id }}"
                                        @if (isset($ded) && $ded->tax_section_item_id == $row->id) selected @endif
                                        @if (isset($ded) && $ded->non_editable == 'yes') disabled @endif>{{ $row->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                ...
                <td class="p-2">
                    <input type="text" @if (isset($ded) && $ded->non_editable == 'yes') disabled @endif name="amount[]"
                        class="form-input price text-end" value="{{ $ded->amount ?? '' }}" required>
                </td>
                <td class="p-2">
                ...
                <td class="p-2 text-center">
                    ...
                    @else
                        @if (isset($ded) && $ded->non_editable == 'yes')
                        @else
                            <i class="fa fa-trash p-2 text-danger" role="button"
                                onclick="return removeDeductionRow(this)"></i>
                        @endif
                    @endif
                </td>
            </tr>
