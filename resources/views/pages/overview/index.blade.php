@extends('layouts.template')
@section('breadcrum')
    @include('layouts.parts.breadcrum')
@endsection
@section('content')
    <div class="card">
        <div class="content d-flex flex-column flex-column-fluid" id="kt_content">

            <div class="post d-flex flex-column-fluid" id="kt_post">
                <!--begin::Container-->
                <div id="kt_content_container" class="container-xxl">
                    <!--begin::Navbar-->

                    <div class="card mb-5 mb-xl-10">
                        <div class="card-body pt-9 pb-0">
                            <!--begin::Details-->
                            <div class="d-flex flex-wrap flex-sm-nowrap mb-3">
                                <!--begin: Pic-->
                                <div class="me-7 mb-4">
                                    <div class="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
                                        {{-- <img src="../assets/media/avatars/300-1.jpg" alt="image" /> --}}
                                        @if (isset(auth()->user()->image) && !empty(auth()->user()->image))
                                            <img alt="user" src="{{ asset('/') . auth()->user()->image }}" />
                                        @else
                                            <img alt="user" src="{{ asset('assets/images/no_Image.jpg') }}" />
                                        @endif
                                        <div
                                            class="position-absolute translate-middle bottom-0 start-100 mb-6 bg-success rounded-circle border border-4 border-white h-20px w-20px">
                                        </div>
                                    </div>
                                </div>
                                <!--end::Pic-->
                                <!--begin::Info-->
                                <div class="flex-grow-1">
                                    <!--begin::Title-->
                                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                                        <!--begin::User-->
                                        <div class="d-flex flex-column">

                                            <!--begin::Name-->
                                            <div class="d-flex align-items-center mb-2">
                                                <a href="#"
                                                    class="text-gray-900 text-hover-primary fs-2 fw-bolder me-1">{{ auth()->user()->name }}</a>
                                                <a href="#"
                                                    class="btn btn-sm btn-light-success fw-bolder ms-2 fs-8 py-1 px-3">Active</a>
                                            </div>
                                            <!--end::Name-->
                                            <!--begin::Info-->
                                            <div class="d-flex flex-wrap fw-bold fs-6 mb-4 pe-2">
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/communication/com006.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg version="1.1" id="Capa_1"
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                                            y="0px" viewBox="0 0 436.558 436.558"
                                                            style="enable-background:new 0 0 436.558 436.558;"
                                                            xml:space="preserve">
                                                            <path
                                                                d="M335.654,239.048l-55.617-16.857c-1.344-4.064-4.323-7.579-8.491-10.555c4.585-5.094,8.455-10.251,11.397-14.517
                                                c6.979-10.122,12.79-21.023,16.813-31.541c1.98-4.436,3.67-8.889,5.038-13.259c5.415-4.538,8.603-11.318,8.603-18.429v-17.418
                                                c0-5-1.572-9.888-4.464-13.941V78.01c0-43.019-34.996-78.01-78.01-78.01h-25.288c-43.014,0-78.01,34.991-78.01,78.01v24.523
                                                c-2.892,4.053-4.463,8.944-4.463,13.939v17.418c0,7.105,3.188,13.88,8.603,18.426c1.366,4.365,3.057,8.822,5.038,13.269
                                                c4.025,10.514,9.834,21.413,16.812,31.528c2.943,4.268,6.813,9.426,11.399,14.522c-4.168,2.976-7.148,6.492-8.492,10.555
                                                l-55.617,16.857c-39.159,11.254-62.625,180.444-62.625,180.444h0.003c-0.118,0.752-0.197,1.517-0.197,2.302
                                                c0,8.154,6.609,14.764,14.764,14.764h330.86c8.155,0,14.764-6.609,14.764-14.764C398.473,414.715,374.813,250.302,335.654,239.048z
                                                M204.79,252.411c0.948,1.051,1.406,2.454,1.261,3.862l-2.74,26.575c-0.211,2.045-1.652,3.752-3.632,4.304
                                                c-0.444,0.124-0.895,0.184-1.341,0.184c-1.544,0-3.032-0.717-3.993-1.99l-14.05-18.636c-0.654-0.868-1.008-1.926-1.007-3.013
                                                l0.018-29.881c0.001-1.752,0.919-3.376,2.421-4.281c1.502-0.904,3.367-0.956,4.916-0.137c4.09,2.162,8.174,3.615,12.139,4.319
                                                c2.387,0.423,4.126,2.499,4.126,4.923v12.467C203.619,251.386,204.266,251.83,204.79,252.411z M230.506,256.272
                                                c-0.145-1.408,0.313-2.811,1.261-3.862c0.524-0.581,1.171-1.024,1.883-1.304V238.64c0-2.424,1.739-4.499,4.126-4.923
                                                c3.963-0.704,8.046-2.156,12.135-4.317c1.55-0.818,3.415-0.766,4.917,0.138c1.502,0.905,2.419,2.529,2.419,4.282l0.003,29.88
                                                c0,1.086-0.353,2.142-1.007,3.009l-14.031,18.623c-0.96,1.274-2.449,1.992-3.993,1.992c-0.446,0-0.897-0.06-1.341-0.184
                                                c-1.98-0.551-3.422-2.259-3.633-4.304L230.506,256.272z M157.286,140.165l-0.271-2.071l-4.852-3.153
                                                c-1.419-0.922-2.275-2.5-2.275-4.192v-8.232c0-2.761,2.239-5,5-5h2.043v-8.916c0-1.895,1.071-3.627,2.767-4.474
                                                c6.884-3.437,20.544-9.202,34.688-9.202c11.285,0,20.642,3.754,27.812,11.157c8.933,9.233,19.229,13.914,30.604,13.914
                                                c6.439,0,13.094-1.52,19.782-4.516c1.546-0.693,3.339-0.555,4.763,0.366c0.665,0.431,1.208,1.007,1.597,1.67h2.728
                                                c2.761,0,5,2.239,5,5v8.232c0,1.692-0.856,3.27-2.275,4.192l-4.852,3.154l-0.27,2.059c-1.58,11.956-8.287,27.444-17.942,41.438
                                                c-12.226,17.733-23.687,25.671-29.62,25.671h-26.862c-5.934,0-17.395-7.939-29.622-25.675
                                                C165.572,167.599,158.865,152.113,157.286,140.165z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </span>
                                                    <!--end::Svg Icon-->Male
                                                </a>
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/communication/com006.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            height="24" viewBox="0 0 24 24" fill="none">
                                                            <path opacity="0.3"
                                                                d="M22 12C22 17.5 17.5 22 12 22C6.5 22 2 17.5 2 12C2 6.5 6.5 2 12 2C17.5 2 22 6.5 22 12ZM12 7C10.3 7 9 8.3 9 10C9 11.7 10.3 13 12 13C13.7 13 15 11.7 15 10C15 8.3 13.7 7 12 7Z"
                                                                fill="currentColor" />
                                                            <path
                                                                d="M12 22C14.6 22 17 21 18.7 19.4C17.9 16.9 15.2 15 12 15C8.8 15 6.09999 16.9 5.29999 19.4C6.99999 21 9.4 22 12 22Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </span>
                                                    <!--end::Svg Icon-->Teacher
                                                </a>
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/general/gen018.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            height="24" viewBox="0 0 24 24" fill="none">
                                                            <path opacity="0.3"
                                                                d="M18.0624 15.3453L13.1624 20.7453C12.5624 21.4453 11.5624 21.4453 10.9624 20.7453L6.06242 15.3453C4.56242 13.6453 3.76242 11.4453 4.06242 8.94534C4.56242 5.34534 7.46242 2.44534 11.0624 2.04534C15.8624 1.54534 19.9624 5.24534 19.9624 9.94534C20.0624 12.0453 19.2624 13.9453 18.0624 15.3453Z"
                                                                fill="currentColor" />
                                                            <path
                                                                d="M12.0624 13.0453C13.7193 13.0453 15.0624 11.7022 15.0624 10.0453C15.0624 8.38849 13.7193 7.04535 12.0624 7.04535C10.4056 7.04535 9.06241 8.38849 9.06241 10.0453C9.06241 11.7022 10.4056 13.0453 12.0624 13.0453Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </span>
                                                    <!--end::Svg Icon-->Chennai
                                                </a>
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/general/gen018.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" x="0px"
                                                            y="0px" width="100px" height="100px"
                                                            viewBox="0 0 100 100" enable-background="new 0 0 100 100"
                                                            xml:space="preserve">
                                                            <path fill="currentColor"
                                                                d="M77.7,63.9l-6.2-5c-2.1-1.7-5.1-1.801-7.3-0.2L58.3,63c-0.8,0.6-1.899,0.5-2.6-0.2L46,54l-8.9-9.8
                                    c-0.7-0.7-0.8-1.8-0.2-2.6l4.3-5.9c1.6-2.2,1.5-5.2-0.2-7.3l-5-6.2c-2.2-2.8-6.4-3-8.9-0.5l-5.4,5.4c-1.2,1.2-1.8,2.9-1.8,4.5
                                    c0.7,12.7,6.5,24.8,15,33.3s20.5,14.3,33.3,15c1.7,0.1,3.3-0.601,4.5-1.801L78.1,72.7C80.8,70.3,80.5,66.1,77.7,63.9z" />
                                                        </svg>
                                                    </span>
                                                    <!--end::Svg Icon-->9566001488
                                                </a>
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/general/gen018.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg version="1.1" id="Capa_1"
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                                            y="0px" viewBox="0 0 489.2 489.2"
                                                            style="enable-background:new 0 0 489.2 489.2;"
                                                            xml:space="preserve">
                                                            <path fill="currentColor"
                                                                d="M177.8,238.1c0,4.5-3.6,8.1-8.1,8.1h-30.4c-4.5,0-8.1-3.6-8.1-8.1v-30.4c0-4.5,3.6-8.1,8.1-8.1h30.4
                                    c4.5,0,8.1,3.6,8.1,8.1V238.1z M241.3,207.8c0-4.5-3.6-8.1-8.1-8.1h-30.4c-4.5,0-8.1,3.6-8.1,8.1v30.4c0,4.5,3.6,8.1,8.1,8.1h30.4
                                    c4.5,0,8.1-3.6,8.1-8.1V207.8z M304.8,207.8c0-4.5-3.6-8.1-8.1-8.1h-30.4c-4.5,0-8.1,3.6-8.1,8.1v30.4c0,4.5,3.6,8.1,8.1,8.1h30.4
                                    c4.5,0,8.1-3.6,8.1-8.1V207.8z M177.8,269.6c0-4.5-3.6-8.1-8.1-8.1h-30.4c-4.5,0-8.1,3.6-8.1,8.1V300c0,4.5,3.6,8.1,8.1,8.1h30.4
                                    c4.5,0,8.1-3.6,8.1-8.1V269.6z M241.3,269.6c0-4.5-3.6-8.1-8.1-8.1h-30.4c-4.5,0-8.1,3.6-8.1,8.1V300c0,4.5,3.6,8.1,8.1,8.1h30.4
                                    c4.5,0,8.1-3.6,8.1-8.1V269.6z M296.7,261.5h-30.4c-4.5,0-8.1,3.6-8.1,8.1V300c0,4.5,3.6,8.1,8.1,8.1h30.4c4.5,0,8.1-3.6,8.1-8.1
                                    v-30.4C304.8,265.1,301.2,261.5,296.7,261.5z M106.1,323.3H75.8c-4.5,0-8.1,3.6-8.1,8.1v30.4c0,4.5,3.6,8.1,8.1,8.1h30.4
                                    c4.5,0,8.1-3.6,8.1-8.1v-30.4C114.3,326.9,110.6,323.3,106.1,323.3z M114.3,269.6c0-4.5-3.6-8.1-8.1-8.1H75.8
                                    c-4.5,0-8.1,3.6-8.1,8.1V300c0,4.5,3.6,8.1,8.1,8.1h30.4c4.5,0,8.1-3.6,8.1-8.1V269.6z M233.2,323.3h-30.4c-4.5,0-8.1,3.6-8.1,8.1
                                    v30.4c0,4.5,3.6,8.1,8.1,8.1h30.4c4.5,0,8.1-3.6,8.1-8.1v-30.4C241.3,326.9,237.7,323.3,233.2,323.3z M169.7,323.3h-30.4
                                    c-4.5,0-8.1,3.6-8.1,8.1v30.4c0,4.5,3.6,8.1,8.1,8.1h30.4c4.5,0,8.1-3.6,8.1-8.1v-30.4C177.8,326.9,174.2,323.3,169.7,323.3z
                                    M360.2,246.3c4.5,0,8.1-3.6,8.1-8.1v-30.4c0-4.5-3.6-8.1-8.1-8.1h-30.4c-4.5,0-8.1,3.6-8.1,8.1v30.4c0,4.5,3.6,8.1,8.1,8.1H360.2
                                    z M47.7,435.9h230.7c-3.7-11.6-5.8-24-5.9-36.8H47.7c-6,0-10.8-4.9-10.8-10.8V171h361.7v101.1c12.8,0.1,25.2,2,36.8,5.7V94.9
                                    c0-26.3-21.4-47.7-47.7-47.7h-53.4V17.8c0-9.6-7.8-17.4-17.4-17.4h-27.1c-9.6,0-17.4,7.8-17.4,17.4v29.5H163V17.8
                                    c0-9.6-7.8-17.4-17.4-17.4h-27.1c-9.6,0-17.4,7.8-17.4,17.4v29.5H47.7C21.4,47.3,0,68.7,0,95v293.3C0,414.5,21.4,435.9,47.7,435.9
                                    z M489.2,397.7c0,50.3-40.8,91.1-91.1,91.1S307,448,307,397.7s40.8-91.1,91.1-91.1S489.2,347.4,489.2,397.7z M444.1,374.1
                                    c0-2.9-1.1-5.7-3.2-7.7c-4.3-4.3-11.2-4.3-15.5,0L385.8,406l-15.2-15.2c-4.3-4.3-11.2-4.3-15.5,0c-2.1,2.1-3.2,4.8-3.2,7.7
                                    c0,2.9,1.1,5.7,3.2,7.7l22.9,22.9c4.3,4.3,11.2,4.3,15.5,0l47.3-47.3C443,379.8,444.1,377,444.1,374.1z" />
                                                        </svg>
                                                    </span>
                                                    <!--end::Svg Icon-->18/09/1993
                                                </a>
                                                <a href="#"
                                                    class="d-flex align-items-center text-gray-400 text-hover-primary mb-2">
                                                    <!--begin::Svg Icon | path: icons/duotune/communication/com011.svg-->
                                                    <span class="svg-icon svg-icon-4 me-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            height="24" viewBox="0 0 24 24" fill="none">
                                                            <path opacity="0.3"
                                                                d="M21 19H3C2.4 19 2 18.6 2 18V6C2 5.4 2.4 5 3 5H21C21.6 5 22 5.4 22 6V18C22 18.6 21.6 19 21 19Z"
                                                                fill="currentColor" />
                                                            <path
                                                                d="M21 5H2.99999C2.69999 5 2.49999 5.10005 2.29999 5.30005L11.2 13.3C11.7 13.7 12.4 13.7 12.8 13.3L21.7 5.30005C21.5 5.10005 21.3 5 21 5Z"
                                                                fill="currentColor" />
                                                        </svg>
                                                    </span>
                                                    {{ auth()->user()->email }}
                                                </a>
                                            </div>
                                            <!--end::Info-->
                                        </div>
                                        <!--end::User-->
                                        <!--begin::Actions-->

                                        <!--end::Actions-->
                                    </div>
                                    <!--end::Title-->
                                    <!--begin::Stats-->
                                    <div class="d-flex flex-wrap flex-stack">
                                        <!--begin::Wrapper-->
                                        <div class="d-flex flex-column flex-grow-1 pe-8">
                                            <!--begin::Stats-->
                                            <div class="d-flex flex-wrap">
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <!--end::Svg Icon-->
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="12589" data-kt-countup-prefix="">12589
                                                        </div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Employe ID</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--end::Stat-->
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <!--end::Svg Icon-->
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="08">Smith</div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Reporting Person</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <!--end::Svg Icon-->
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="08">Married</div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Marital Status</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="2000">Teaching</div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Employee category</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="2000">1983</div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Employed Since</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--end::Stat-->
                                                <!--begin::Stat-->
                                                <div
                                                    class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6 mb-3">
                                                    <!--begin::Number-->
                                                    <div class="d-flex align-items-center">
                                                        <!--end::Svg Icon-->
                                                        <div class="fs-2 fw-bolder" data-kt-countup="false"
                                                            data-kt-countup-value="08">10</div>
                                                    </div>
                                                    <!--end::Number-->
                                                    <!--begin::Label-->
                                                    <div class="fw-bold fs-6 text-gray-400">Total Experience</div>
                                                    <!--end::Label-->
                                                </div>
                                                <!--end::Stat-->
                                                <!--end::Stat-->
                                            </div>
                                            <!--end::Stats-->
                                        </div>
                                        <!--end::Wrapper-->
                                        <!--begin::Progress-->
                                        <!--end::Progress-->
                                    </div>
                                    <!--end::Stats-->
                                </div>
                                <!--end::Info-->
                            </div>
                            <!--end::Details-->
                            <!--begin::Navs-->
                        </div>
                    </div>
                    <!--end::Navbar-->
                    <!--begin::details View-->
                    <div class="card pt-2 mb-6 mb-xl-9">
                        <!--begin::Card header-->
                        <div class="card-header border-0">
                            <!--end::Card title-->
                            <!--begin::Toolbar-->
                            <div class="card-toolbar m-0">
                                <!--begin::Tab nav-->
                                <ul class="nav nav-stretch fs-5 fw-bolder nav-line-tabs nav-line-tabs-2x border-transparent"
                                    role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_year_tab" class="nav-link text-active-primary  active"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_1"
                                            aria-selected="True">Overview</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2019_tab" class="nav-link text-active-primary ms-3"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_2"
                                            aria-selected="false">Personal Info</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2018_tab" class="nav-link text-active-primary ms-3"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_3"
                                            aria-selected="false">Family Info</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2017_tab" class="nav-link text-active-primary"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_4"
                                            aria-selected="false">Academics Info</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2016_tab" class="nav-link text-active-primary"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_5"
                                            aria-selected="false">Leave Info</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2015_tab" class="nav-link text-active-primary"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_6"
                                            aria-selected="false">Utilities</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="kt_referrals_2014_tab" class="nav-link text-active-primary"
                                            data-bs-toggle="tab" role="tab" href="#kt_customer_details_invoices_7"
                                            aria-selected="false">Others</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a id="password_tab" class="nav-link text-active-primary" data-bs-toggle="tab"
                                            role="tab" href="#kt_customer_password_tab" aria-selected="false">Change
                                            Password</a>
                                    </li>
                                </ul>
                                <!--end::Tab nav-->
                            </div>
                            <!--end::Toolbar-->
                        </div>
                        <!--end::Card header-->
                        <!--begin::Card body-->
                        <div class="card-body pt-0">

                            <br>
                            <!--begin::Tab Content-->
                            <div id="kt_referred_users_tab_content" class="tab-content">
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_1" class="py-0 tab-pane fade active show"
                                    role="tabpanel">
                                    <!--begin::Table-->
                                    <div id="kt_customer_details_invoices_table_1_wrapper"
                                        class="dataTables_wrapper dt-bootstrap4 no-footer">
                                        <div class="row g-6 g-xl-9">

                                            <div class="col-lg-6 col-xxl-4">
                                                <!--begin::Card-->
                                                <div class="card h-100 cstmzed">
                                                    <!--begin::Card body-->
                                                    <div class="card-body p-9">
                                                        <!--begin::Heading-->
                                                        <div class="fs-2hx fw-bolder">150</div>
                                                        <div class="fs-4 fw-bold text-gray-400 mb-7">Total for the Months
                                                        </div>
                                                        <!--end::Heading-->
                                                        <!--begin::Wrapper-->
                                                        <!--begin::Chart-->
                                                        <div class="d-flex flex-center me-9 mb-5">
                                                            <canvas id="kt_project_list_chart-1"
                                                                style="display: block; box-sizing: border-box; height:150px; width: 150px;"
                                                                width="150" height="150"></canvas>
                                                        </div>
                                                        <div class="d-flex flex-wrap">

                                                            <!--end::Chart-->
                                                            <!--begin::Labels-->
                                                            <div
                                                                class="d-flex flex-column justify-content-center flex-row-fluid pe-11 mb-5">
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center mb-3">
                                                                    <div class="bullet bg-primary me-3"></div>
                                                                    <div class="text-gray-400">Working days</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">120</div>
                                                                </div>
                                                                <!--end::Label-->
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center mb-3">
                                                                    <div class="bullet bg-success me-3"></div>
                                                                    <div class="text-gray-400">Present</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">110</div>
                                                                </div>
                                                                <!--end::Label-->
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center">
                                                                    <div class="bullet bg-gray-300 me-3"></div>
                                                                    <div class="text-gray-400">Absent</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">10</div>
                                                                </div>
                                                                <!--end::Label-->
                                                            </div>
                                                            <!--end::Labels-->
                                                        </div>
                                                        <!--end::Wrapper-->
                                                    </div>
                                                    <!--end::Card body-->
                                                </div>
                                                <!--end::Card-->
                                            </div>
                                            <div class="col-lg-6 col-xxl-4">
                                                <!--begin::Card-->
                                                <div class="card h-100 cstmzed">
                                                    <!--begin::Card body-->
                                                    <div class="card-body p-9">
                                                        <!--begin::Heading-->
                                                        <div class="fs-2hx fw-bolder">22</div>
                                                        <div class="fs-4 fw-bold text-gray-400 mb-7">Total Leaves</div>
                                                        <!--end::Heading-->
                                                        <!--begin::Wrapper-->
                                                        <!--begin::Chart-->
                                                        <div class="d-flex flex-center me-9 mb-5">
                                                            <canvas id="kt_project_list_chart-2"
                                                                style="display: block; box-sizing: border-box; height:150px; width: 150px;"
                                                                width="150" height="150"></canvas>
                                                        </div>
                                                        <div class="d-flex flex-wrap">

                                                            <!--end::Chart-->
                                                            <!--begin::Labels-->
                                                            <div
                                                                class="d-flex flex-column justify-content-center flex-row-fluid pe-11 mb-5">
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center mb-3">
                                                                    <div class="bullet bg-primary me-3"></div>
                                                                    <div class="text-gray-400">Allocated</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">8</div>
                                                                </div>
                                                                <!--end::Label-->
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center mb-3">
                                                                    <div class="bullet bg-success me-3"></div>
                                                                    <div class="text-gray-400">Taken</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">4</div>
                                                                </div>
                                                                <!--end::Label-->
                                                                <!--begin::Label-->
                                                                <div class="d-flex fs-6 fw-bold align-items-center">
                                                                    <div class="bullet bg-gray-300 me-3"></div>
                                                                    <div class="text-gray-400">Balance</div>
                                                                    <div class="ms-auto fw-bolder text-gray-700">4</div>
                                                                </div>
                                                                <!--end::Label-->
                                                            </div>
                                                            <!--end::Labels-->
                                                        </div>
                                                        <!--end::Wrapper-->
                                                    </div>
                                                    <!--end::Card body-->
                                                </div>
                                                <!--end::Card-->
                                            </div>
                                            <div class="col-lg-6 col-xxl-4">
                                                <!--begin::Budget-->
                                                <div class="card h-100 cstmzed">
                                                    <div class="card-body p-9">
                                                        <div class="fs-2hx fw-bolder">₹3,50,000.00</div>
                                                        <div class="fs-4 fw-bold text-gray-400 mb-7">Total Loan Taken</div>
                                                        <div class="fs-6 d-flex justify-content-between mb-4">
                                                            <div class="fw-bold">Paid</div>
                                                            <div class="d-flex fw-bolder">
                                                                <!--begin::Svg Icon | path: icons/duotune/arrows/arr007.svg-->
                                                                <span class="svg-icon svg-icon-3 me-1 svg-icon-success">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                                        height="24" viewBox="0 0 24 24"
                                                                        fill="none">
                                                                        <path
                                                                            d="M13.4 10L5.3 18.1C4.9 18.5 4.9 19.1 5.3 19.5C5.7 19.9 6.29999 19.9 6.69999 19.5L14.8 11.4L13.4 10Z"
                                                                            fill="currentColor"></path>
                                                                        <path opacity="0.3"
                                                                            d="M19.8 16.3L8.5 5H18.8C19.4 5 19.8 5.4 19.8 6V16.3Z"
                                                                            fill="currentColor"></path>
                                                                    </svg>
                                                                </span>
                                                                <!--end::Svg Icon-->₹2,20,000.00
                                                            </div>
                                                        </div>
                                                        <div class="separator separator-dashed"></div>
                                                        <div class="fs-6 d-flex justify-content-between my-4">
                                                            <div class="fw-bold">Yet to Paid</div>
                                                            <div class="d-flex fw-bolder">
                                                                <!--begin::Svg Icon | path: icons/duotune/arrows/arr006.svg-->
                                                                <span class="svg-icon svg-icon-3 me-1 svg-icon-danger">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                                        height="24" viewBox="0 0 24 24"
                                                                        fill="none">
                                                                        <path
                                                                            d="M13.4 14.8L5.3 6.69999C4.9 6.29999 4.9 5.7 5.3 5.3C5.7 4.9 6.29999 4.9 6.69999 5.3L14.8 13.4L13.4 14.8Z"
                                                                            fill="currentColor"></path>
                                                                        <path opacity="0.3"
                                                                            d="M19.8 8.5L8.5 19.8H18.8C19.4 19.8 19.8 19.4 19.8 18.8V8.5Z"
                                                                            fill="currentColor"></path>
                                                                    </svg>
                                                                </span>
                                                                <!--end::Svg Icon-->₹1,30,000.00
                                                            </div>
                                                        </div>
                                                        <div class="separator separator-dashed"></div>
                                                        <div class="fs-6 d-flex justify-content-between my-4">
                                                            <div class="fw-bold">Monthly EMI</div>
                                                            <div class="d-flex fw-bolder"> ₹20000 </div>
                                                        </div>
                                                        <div class="separator separator-dashed"></div>
                                                        <div class="fs-6 d-flex justify-content-between my-4">
                                                            <div class="fw-bold">Loan Issued Date</div>
                                                            <div class="d-flex fw-bolder"> 20/01/2021 </div>
                                                        </div>
                                                        <div class="separator separator-dashed"></div>
                                                        <div class="fs-6 d-flex justify-content-between my-4">
                                                            <div class="fw-bold">Loan Start Date</div>
                                                            <div class="d-flex fw-bolder"> 20/02/2021 </div>
                                                        </div>
                                                        <div class="separator separator-dashed"></div>
                                                        <div class="fs-6 d-flex justify-content-between my-4">
                                                            <div class="fw-bold">Loan End Date</div>
                                                            <div class="d-flex fw-bolder"> 20/03/2022 </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <!--end::Budget-->
                                            </div>
                                            <div class="col-xl-6 mb-5 mb-xl-10">
                                                <!--begin::Tables widget 9-->
                                                <div class="card card-flush h-xl-100 cstmzed">
                                                    <!--begin::Header-->
                                                    <div class="card-header pt-10">
                                                        <!--begin::Title-->
                                                        <h1 class="text-dark mb-5 fw-bolder">Languages Known</h1>
                                                        <!--end::Title-->
                                                    </div>
                                                    <!--end::Header-->
                                                    <!--begin::Body-->
                                                    <div class="card-body py-3">
                                                        <!--begin::Table container-->
                                                        <div class="table-responsive">
                                                            <!--begin::Table-->
                                                            <table class="table table-row-dashed align-middle gs-0 gy-4">
                                                                <!--begin::Table head-->
                                                                <thead>
                                                                    <tr class="fs-5 fw-bold border-0 text-gray-400">
                                                                        <th class="">Languages</th>
                                                                        <th class="text-center">Is Read</th>
                                                                        <th class="text-center">Is Write</th>
                                                                        <th class="text-center">Is Speak</th>
                                                                    </tr>
                                                                </thead>
                                                                <!--end::Table head-->
                                                                <!--begin::Table body-->
                                                                <tbody>
                                                                    <tr>
                                                                        <td class="">
                                                                            <a href="#"
                                                                                class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">Tamil</a>
                                                                        </td>
                                                                        <td class="pe-0">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="">
                                                                            <a href="#"
                                                                                class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">English</a>
                                                                        </td>
                                                                        <td class="pe-0">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span class="svg-icon svg-icon-1">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="7"
                                                                                            y="15.3137" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(-45 7 15.3137)"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="8.41422"
                                                                                            y="7" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(45 8.41422 7)"
                                                                                            fill="currentColor"></rect>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="">
                                                                            <a href="#"
                                                                                class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">Hindi</a>
                                                                        </td>
                                                                        <td class="pe-0">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span class="svg-icon svg-icon-1">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="7"
                                                                                            y="15.3137" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(-45 7 15.3137)"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="8.41422"
                                                                                            y="7" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(45 8.41422 7)"
                                                                                            fill="currentColor"></rect>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td class="">
                                                                            <a href="#"
                                                                                class="text-gray-800 fw-bold text-hover-primary mb-1 fs-6">French</a>
                                                                        </td>
                                                                        <td class="pe-0">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span
                                                                                    class="svg-icon svg-icon-1 svg-icon-success">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <path
                                                                                            d="M10.4343 12.4343L8.75 10.75C8.33579 10.3358 7.66421 10.3358 7.25 10.75C6.83579 11.1642 6.83579 11.8358 7.25 12.25L10.2929 15.2929C10.6834 15.6834 11.3166 15.6834 11.7071 15.2929L17.25 9.75C17.6642 9.33579 17.6642 8.66421 17.25 8.25C16.8358 7.83579 16.1642 7.83579 15.75 8.25L11.5657 12.4343C11.2533 12.7467 10.7467 12.7467 10.4343 12.4343Z"
                                                                                            fill="currentColor"></path>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                        <td class="">
                                                                            <div class="d-flex justify-content-center">
                                                                                <span class="svg-icon svg-icon-1">
                                                                                    <svg width="24" height="24"
                                                                                        viewBox="0 0 24 24" fill="none"
                                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                                        <rect opacity="0.3"
                                                                                            x="2" y="2"
                                                                                            width="20" height="20"
                                                                                            rx="10"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="7"
                                                                                            y="15.3137" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(-45 7 15.3137)"
                                                                                            fill="currentColor"></rect>
                                                                                        <rect x="8.41422"
                                                                                            y="7" width="12"
                                                                                            height="2" rx="1"
                                                                                            transform="rotate(45 8.41422 7)"
                                                                                            fill="currentColor"></rect>
                                                                                    </svg>
                                                                                </span>
                                                                            </div>
                                                                        </td>
                                                                    </tr>

                                                                </tbody>
                                                                <!--end::Table body-->
                                                            </table>
                                                            <!--end::Table-->
                                                        </div>
                                                        <!--end::Table container-->
                                                    </div>
                                                    <!--end::Body-->
                                                </div>
                                                <!--end::Tables Widget 9-->
                                            </div>
                                            <br>
                                        </div>
                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_2" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
                                        <!--begin::Card header-->
                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0">
                                                <h3 class="fw-bold m-0">Personal Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <!--begin::Card header-->
                                        <!--begin::Card body-->
                                        <div class="row">
                                            <div class="col-lg-6">

                                                <div class="card-body cstmzed-witpad">
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Place of
                                                            Birth</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Chennai</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">MotherTongue</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Tamil</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Nationality </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> India </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Religion </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> Christianity
                                                            </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Community </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 fv-row">
                                                            <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Caste</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <a href="#"
                                                                class="fw-semibold fs-6 text-gray-800 text-hover-primary">Nadar</a>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Pan No </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 0123456789 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Aadhaar No</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 0123456789 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Date of Retirement
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 20/02/2030 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Date of Joining
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 20/02/2070 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Blood Group</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> A+ve </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Disorders </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> Diabetes </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Weight</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">80</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Height</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">5.5</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Emergency Contact
                                                            No</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">1020304050</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Doctor</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Dr.Alagappan</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Doctor
                                                            Address</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Chennai</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Doctor Phone
                                                            Number</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 0123456789 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">ID Marks</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> Thumb finger cut
                                                            </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">IsHandicap</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> No </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> HandicapDetails
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Row-->
                                                    </div>
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> BankStatus </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> Active </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Bank Name </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> SBI </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Bank Branch Code
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> IFSC000123
                                                            </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                </div>
                                                <!--end::Notice-->
                                            </div>

                                            <div class="col-lg-6">

                                                <div class="card-body cstmzed-witpad">

                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Account Number
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> 1234567890123
                                                            </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> EPF Code </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 fv-row">
                                                            <span class="fw-bold text-gray-800 fs-6"> 123456 987</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">SMS Copy</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <a href="#"
                                                                class="fw-bold fs-6 text-gray-800 text-hover-primary"> 2121
                                                                5454 </a>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Present Status
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Married</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Ration Card No
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 20203256 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Smart Card No
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 205850326 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> File No </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 205 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Locker No</label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> DFV25639 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Department </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> Physics </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Designation
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> Teacher </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Working Count
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 2000 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Sisters Count
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 100 </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Building </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> B Block</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Place of work
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">2'nd Floor</span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Transfer Reference
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> UAN </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Name of Institution
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Nature of
                                                            Employment </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Other Talents
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Allergy </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Attendance scheme
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Disease </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Differently Ables
                                                        </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Remarks </label>
                                                        <!--begin::Label-->
                                                        <!--begin::Label-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> - </span>
                                                        </div>
                                                        <!--begin::Label-->
                                                    </div>
                                                    <!--end::Notice-->
                                                </div>
                                            </div>
                                        </div>

                                        <div class="row">
                                            <div class="col-lg-6">
                                                <div class="  cursor-pointer">
                                                    <!--begin::Card title-->
                                                    <div class="card-title m-0">
                                                        <h3 class="fw-bold m-0">Salary Details</h3>
                                                    </div>
                                                    <!--end::Card title-->
                                                    <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                                    <!--end::Action-->
                                                </div>
                                                <div class="card-body cstmzed-witpad">
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">For Month</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">50,000</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Salary Head
                                                            ID</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Tamil</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Salary </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> 50,000 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Academic ID
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> 1025 528 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->

                                                </div>
                                                <!--end::Notice-->
                                            </div>


                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Appointment details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">DOJ
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Salary
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Period
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Month
                                                            from - To</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Probation</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Appointment Order Model</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Appointment Order Upload</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Feb 09, 1993</td>
                                                        <td>50,000</td>
                                                        <td>2020</td>
                                                        <td>2000 - 2022</td>
                                                        <td>Incorporate</td>
                                                        <td>Digitalized</td>
                                                        <td>Paper Document</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Feb 09, 1993</td>
                                                        <td>50,000</td>
                                                        <td>2020</td>
                                                        <td>2000 - 2022</td>
                                                        <td>Incorporate</td>
                                                        <td>Digitalized</td>
                                                        <td>Paper Document</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Locker Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Institution Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Document Type</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Document Upload Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Staff
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Remarks</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Loyola</td>
                                                        <td>PDF</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Boby</td>
                                                        <td>Locker key is with</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Loyola</td>
                                                        <td>PDF</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Boby</td>
                                                        <td>Locker key is with</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Medical Records</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Reason
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Record
                                                            Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Path</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Daniel</td>
                                                        <td>Chennai</td>
                                                        <td>Brother</td>
                                                        <td>Feb 09, 1993</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Jenifer</td>
                                                        <td>Chennai</td>
                                                        <td>Sister</td>
                                                        <td>Feb 09, 1993</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                        <!--end::Card body-->
                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_3" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div id="kt_customer_details_invoices_table_3_wrapper"
                                        class="dataTables_wrapper dt-bootstrap4 no-footer">
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Family Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle gy-4">
                                                <!--begin::Thead-->
                                                <thead class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Relationship</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Name
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">DOB
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Sex
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Qualification</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Occupation</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Address</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Contact No</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Amount: activate to sort column ascending">
                                                            Nationality</th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">Remarks
                                                        </th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Brother</td>
                                                        <td>Jack</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Male</td>
                                                        <td>B.E</td>
                                                        <td>Teacher</td>
                                                        <td>Chennai</td>
                                                        <td>9962589632</td>
                                                        <td>Indian</td>
                                                        <td> - </td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Sister</td>
                                                        <td>Rose</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Female</td>
                                                        <td>Bsc</td>
                                                        <td>Teacher</td>
                                                        <td>Chennai</td>
                                                        <td>9962589632</td>
                                                        <td>Indian</td>
                                                        <td> - </td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Nominees Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Name
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Address</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Relationship with </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">DOB
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Sex
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Share
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Type
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Contact No</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Daniel</td>
                                                        <td>Chennai</td>
                                                        <td>Brother</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Male</td>
                                                        <td>-</td>
                                                        <td>-</td>
                                                        <td>9856321471</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Jenifer</td>
                                                        <td>Chennai</td>
                                                        <td>Sister</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Female</td>
                                                        <td>-</td>
                                                        <td>-</td>
                                                        <td>9856321471</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>


                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_4" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div id="kt_customer_details_invoices_table_4_wrapper"
                                        class="dataTables_wrapper dt-bootstrap4 no-footer">
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Academics Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle gy-4">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Course</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            University</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Year
                                                            Completed</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Main
                                                            Sub</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Ancillary Sub</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Cert
                                                            No</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Submitted Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Returned Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Amount: activate to sort column ascending">Type
                                                        </th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">File Path
                                                        </th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">File Name
                                                        </th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Brother</td>
                                                        <td>Jack</td>
                                                        <td>1993</td>
                                                        <td>Male</td>
                                                        <td>B.E</td>
                                                        <td>Teacher</td>
                                                        <td>Chennai</td>
                                                        <td> Feb 09, 1993 </td>
                                                        <td>Indian</td>
                                                        <td> - </td>
                                                        <td> - </td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Sister</td>
                                                        <td>Brother</td>
                                                        <td>Jack</td>
                                                        <td>1993</td>
                                                        <td>Male</td>
                                                        <td>B.E</td>
                                                        <td>Teacher</td>
                                                        <td> Feb 09, 1993 </td>
                                                        <td>9962589632</td>
                                                        <td>Indian</td>
                                                        <td> - </td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Academic Services</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Academic Year</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Designation</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Department</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Place
                                                            of Work</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Subject Handling</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Class
                                                            Allocated</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Mode
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Languages</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Studied Upto</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Daniel</td>
                                                        <td>Chennai</td>
                                                        <td>Brother</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Male</td>
                                                        <td>-</td>
                                                        <td>-</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Jenifer</td>
                                                        <td>Chennai</td>
                                                        <td>Sister</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Female</td>
                                                        <td>-</td>
                                                        <td>-</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Career</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Order
                                                            No</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Career Service Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Designation</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Place
                                                            of Work</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Probation Period</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Completion Order No</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Completion Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Acd
                                                            From Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Acd
                                                            To Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Building</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Academic ID</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Description</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Salary Description</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>102502</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Teacher</td>
                                                        <td>6 Months</td>
                                                        <td>102356</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>-</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Feb 09, 2003</td>
                                                        <td>B Block</td>
                                                        <td>852963</td>
                                                        <td>Nothing</td>
                                                        <td>Lorem Ipsum</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Jenifer</td>
                                                        <td>Chennai</td>
                                                        <td>Sister</td>
                                                        <td>Feb 09, 1993</td>
                                                        <td>Female</td>
                                                        <td>-</td>
                                                        <td>-</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                        <td>9856321471</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Qualification details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Course</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Universitty</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Main
                                                            Sub</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Year
                                                            of Completion</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Engineering</td>
                                                        <td>Anna University</td>
                                                        <td>Computer</td>
                                                        <td>2021</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Engineering</td>
                                                        <td>Anna University</td>
                                                        <td>Computer</td>
                                                        <td>2021</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_5" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div id="kt_customer_details_invoices_table_5_wrapper"
                                        class="dataTables_wrapper dt-bootstrap4 no-footer">

                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0">
                                                <h3 class="fw-bold m-0">Attendance Management</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="card-body cstmzed-witpad">
                                            <!--begin::Row-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted">Date</label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7">
                                                    <span class="fw-bold fs-6 text-gray-800">Chennai</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Row-->
                                            <!--begin::Row-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted">Leave Category</label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7">
                                                    <span class="fw-bold fs-6 text-gray-800">Tamil</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Row-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted">Staff Category</label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 d-flex align-items-center">
                                                    <span class="fw-bold fs-6 text-gray-800 me-2"> India </span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> No of days </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 d-flex align-items-center">
                                                    <span class="fw-bold fs-6 text-gray-800 me-2"> Christianity </span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> Check In </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 fv-row">
                                                    <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> Check Out </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 fv-row">
                                                    <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> Remarks </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 fv-row">
                                                    <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> Status - Omit / Warning
                                                </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 fv-row">
                                                    <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->
                                            <!--begin::Input group-->
                                            <div class="row mb-7">
                                                <!--begin::Label-->
                                                <label class="col-lg-5 fw-semibold text-muted"> Is Approved </label>
                                                <!--end::Label-->
                                                <!--begin::Col-->
                                                <div class="col-lg-7 fv-row">
                                                    <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                </div>
                                                <!--end::Col-->
                                            </div>
                                            <!--end::Input group-->

                                        </div>
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Leave Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle gy-4">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Academic Year</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Leave
                                                            Id</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Reason</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">From
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">To
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">No of
                                                            Leaves</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Path</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Amount: activate to sort column ascending">Remarks
                                                        </th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">Approved
                                                            By</th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">
                                                            Accumulated</th>
                                                        <th class="min-w-125px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Date: activate to sort column ascending">Available
                                                            Leave</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>2020</td>
                                                        <td>125 562</td>
                                                        <td>Feaver</td>
                                                        <td>Feb 09, 2022</td>
                                                        <td>Feb 13, 2022</td>
                                                        <td>5</td>
                                                        <td>Leave form</td>
                                                        <td>Location</td>
                                                        <td>Coming soon</td>
                                                        <td>HOD </td>
                                                        <td>Somneone</td>
                                                        <td> 30 </td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>2020</td>
                                                        <td>125 562</td>
                                                        <td>Feaver</td>
                                                        <td>Feb 09, 2022</td>
                                                        <td>Feb 13, 2022</td>
                                                        <td>5</td>
                                                        <td>Leave form</td>
                                                        <td>Location</td>
                                                        <td>Coming soon</td>
                                                        <td>HOD </td>
                                                        <td>Somneone</td>
                                                        <td> 30 </td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>

                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Leave Allocation</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Accumulated</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Available Leave</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Leave
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Academic Year</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>22</td>
                                                        <td>20</td>
                                                        <td>Common</td>
                                                        <td>1993</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>22</td>
                                                        <td>20</td>
                                                        <td>Common</td>
                                                        <td>1993</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>


                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_6" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div id="kt_customer_details_invoices_table_6_wrapper"
                                        class="dataTables_wrapper dt-bootstrap4 no-footer">
                                        <div class="cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Invigilation Details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle gy-4">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Year
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            School Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Place
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Invigilation</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>2020</td>
                                                        <td>Oxford</td>
                                                        <td>London</td>
                                                        <td>University</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>2020</td>
                                                        <td>Oxford</td>
                                                        <td>London</td>
                                                        <td>University</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>



                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Attachment details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Description</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">Title
                                                        </th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Uploaded Date</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Engineering</td>
                                                        <td>Anna University</td>
                                                        <td>Computer</td>
                                                        <td>2021</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Engineering</td>
                                                        <td>Anna University</td>
                                                        <td>Computer</td>
                                                        <td>2021</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Punishment details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Punish Title</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Punish Reason</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Punish Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Path</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Late Coming</td>
                                                        <td>Inobedient</td>
                                                        <td>2022</td>
                                                        <td>Name of the file</td>
                                                        <td>File Location</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Late Coming</td>
                                                        <td>Inobedient</td>
                                                        <td>2022</td>
                                                        <td>Name of the file</td>
                                                        <td>File Location</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                        <div class="  cursor-pointer">
                                            <!--begin::Card title-->
                                            <div class="card-title m-0 mb-5">
                                                <h3 class="fw-bold m-0">Remarks details</h3>
                                            </div>
                                            <!--end::Card title-->
                                            <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                            <!--end::Action-->
                                        </div>
                                        <div class="table-responsive cstmzes-tble">
                                            <table id="kt_customer_details_invoices_table_3"
                                                class="table align-middle table-row-dashed fs-6 fw-bolder gy-5 dataTable no-footer">
                                                <!--begin::Thead-->
                                                <thead
                                                    class="border-bottom border-gray-200 fs-7 text-uppercase fw-bolder">
                                                    <tr class="text-start text-muted bg-light gs-0">
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Remark Title</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Remark Reason</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">
                                                            Remark Date</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Name</th>
                                                        <th class="min-w-100px sorting" tabindex="0"
                                                            aria-controls="kt_customer_details_invoices_table_3"
                                                            rowspan="1" colspan="1" style="width: 0px;"
                                                            aria-label="Order ID: activate to sort column ascending">File
                                                            Path</th>
                                                    </tr>
                                                </thead>
                                                <!--end::Thead-->
                                                <!--begin::Tbody-->
                                                <tbody class="fs-6 fw-bold text-gray-600">

                                                    <tr class="odd">
                                                        <td>Late Coming</td>
                                                        <td>Inobedient</td>
                                                        <td>2022</td>
                                                        <td>Name of the file</td>
                                                        <td>File Location</td>
                                                    </tr>
                                                    <tr class="even">
                                                        <td>Late Coming</td>
                                                        <td>Inobedient</td>
                                                        <td>2022</td>
                                                        <td>Name of the file</td>
                                                        <td>File Location</td>
                                                    </tr>
                                                </tbody>
                                                <!--end::Tbody-->
                                            </table>
                                        </div>
                                    </div>
                                    <!--end::Table-->
                                </div>
                                <!--end::Tab panel-->
                                <!--begin::Tab panel-->
                                <div id="kt_customer_details_invoices_7" class="py-0 tab-pane fade" role="tabpanel">
                                    <!--begin::Table-->
                                    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
                                        <!--begin::Card header-->
                                        <!--begin::Card header-->
                                        <!--begin::Card body-->
                                        <div class="row">
                                            <div class="col-lg-6">
                                                <div class="cursor-pointer">
                                                    <!--begin::Card title-->
                                                    <div class="card-title m-0">
                                                        <h3 class="fw-bold m-0">Resignation Details</h3>
                                                    </div>
                                                    <!--end::Card title-->
                                                    <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                                    <!--end::Action-->
                                                </div>
                                                <div class="card-body cstmzed-witpad">
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Year</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Chennai</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Row-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Reason</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Tamil</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Row-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted">Certificate
                                                            Issued</label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> India </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Issued Date
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> Christianity
                                                            </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> ResignDate
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 fv-row">
                                                            <span class="fw-semibold text-gray-800 fs-6">BC</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->

                                                </div>
                                                <!--end::Notice-->
                                            </div>

                                            <div class="col-lg-6">
                                                <div class="  cursor-pointer">
                                                    <!--begin::Card title-->
                                                    <div class="card-title m-0">
                                                        <h3 class="fw-bold m-0"> Retirement Details</h3>
                                                    </div>
                                                    <!--end::Card title-->
                                                    <!--begin::Action
                                <a href="/metronic8/demo1/../demo1/account/settings.html" class="btn btn-primary align-self-center">Edit Profile</a>-->
                                                    <!--end::Action-->
                                                </div>
                                                <div class="card-body cstmzed-witpad">

                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Retirement Title
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 d-flex align-items-center">
                                                            <span class="fw-bold fs-6 text-gray-800 me-2"> 1234567890123
                                                            </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> Retirement Reason
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7 fv-row">
                                                            <span class="fw-bold text-gray-800 fs-6"> 123456 987</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> RetirementDate
                                                        </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <a href="#"
                                                                class="fw-bold fs-6 text-gray-800 text-hover-primary">
                                                                2121 5454 </a>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> File Name </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800">Married</span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->
                                                    <!--begin::Input group-->
                                                    <div class="row mb-7">
                                                        <!--begin::Label-->
                                                        <label class="col-lg-5 fw-semibold text-muted"> File Path </label>
                                                        <!--end::Label-->
                                                        <!--begin::Col-->
                                                        <div class="col-lg-7">
                                                            <span class="fw-bold fs-6 text-gray-800"> 20203256 </span>
                                                        </div>
                                                        <!--end::Col-->
                                                    </div>
                                                    <!--end::Input group-->

                                                </div>
                                            </div>
                                        </div>
                                        <!--end::Card body-->
                                    </div>
                                    <!--end::Table-->
                                </div>


                                {{-- ////////// --}}



                                <div id="kt_customer_password_tab" class="py-0 tab-pane fade" role="tabpanel">
                                    @include('pages.overview.password.form')
                                </div>



                                {{-- //////////// --}}




                                <!--end::Tab panel-->
                            </div>
                            <!--end::Tab Content-->
                        </div>
                        <!--end::Card body-->
                    </div>
                    <!--end::details View-->


                    <!--end::Row-->
                </div>
                <!--end::Container-->
            </div>
            <!--end::Post-->
        </div>
    </div>
@endsection

@section('add_on_script')
    
@endsection
