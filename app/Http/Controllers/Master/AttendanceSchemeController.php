...
class AttendanceSchemeController extends Controller
{
        ...
        if($request->ajax())
        {
            ...
            ->editColumn('created_at', function ($row) {
              ...
              ->addColumn('action', function ($row) {
                ...
                {
                    $edit_btn = '<a href="javascript:void(0);" onclick="getAttendanceSchemeModal(' . $row->id . ')"  class="btn btn-icon btn-active-primary btn-light-primary mx-1 w-30px h-30px" > 
    ...
    public function add_edit(Request $request)
    {
        ...
        if(isset($id) && !empty($id))
        {
         ...
         $content = view('pages.masters.scheme.add_edit_form',compact('info','title', 'from'));
