...
class BulkImport implements ToCollection, WithHeadingRow, WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            $check_institute_code = Institution::where(
                "code",
                $institute_code
            )->first();
            ...
            $check_division_name = Division::where(
                "name",
                $division_name
            )->first();
            ...
            $aadhar_document_id = $staff_personal = [
            ...
            if ($row["aadhaar_card_name"] and $row["aadhaar_card_no"]) {
                $aadhar_check = DocumentType::Where("name", "Adhaar")
                    ->orWhere("name", "Aadhaar")
                    ->orWhere("name", "Aadhar")
                    ->first();
                if ($aadhar_check) {
                ...
                StaffDocument::updateOrCreate(
                    ["staff_id" => $staff_id, "document_type_id" => $aadhar_id],
                    $staff_aadhaar
                );
            ...
            if ($row["pan_card_name"] and $row["pan_card_name"]) {
                $pancard_check = DocumentType::Where(
                    "name",
                    "Pan Card"
                )->first();
                if ($pancard_check) {
            ...
            if ($row["ration_card_name"] and $row["ration_card_no"]) {
                $rationcard_check = DocumentType::Where(
                    "name",
                    "Ration Card"
                )->first();
                if ($rationcard_check) {
            ...
            if ($row["driving_licence_name"] and $row["driving_licence_no"]) {
                $drivinglicence_check = DocumentType::Where(
                    "name",
                    "Driving License"
                )->first();
                if ($drivinglicence_check) {
                    ...
                    $drivinglicence_id = DocumentType::create($drivinglicence)
                        ->id;
                }
            ...
            if ($row["voter_id_name"] and $row["voter_id_no"]) {
                $voterid_check = DocumentType::Where(
                    "name",
                    "Voter ID"
                )->first();
                if ($voterid_check) {
            ...
            if ($row["passport_name"] and $row["passport_no"]) {
                $passport_check = DocumentType::Where(
                    "name",
                    "Passport"
                )->first();
                if ($passport_check) {
            ...
            $lanuguage_check = Language::where(
                "name",
                $row["mother_tongue"]
            )->first();
            if ($lanuguage_check) {
            ...
            $place_of_birth_check = OtherSchoolPlace::where(
                "name",
                $row["place_of_birth"]
            )->first();
            if ($place_of_birth_check) {
                ...
                $place_of_birth_id = OtherSchoolPlace::create(
                    $place_of_birth_insert
                )->id;
            }
            ...
            $nationality_check = Nationality::where(
                "name",
                $row["nationality"]
            )->first();
            ...
            $community_check = Community::where(
                "name",
                $row["community"]
            )->first();
            if ($community_check) {
            ...
            if ($row["uan_no"]) {
                $uan_insert = [
                    ...
                    "type" => "pf",
                    "start_date" => $row["uan_start_date"]
                        ? date("Y-m-d", strtotime($row["uan_start_date"]))
                        : null,
                    "location" => $row["uan_area"],
                    "status" => "active",
                ];
                StaffPfEsiDetail::updateOrCreate(
                    ["staff_id" => $staff_id, "type" => "pf"],
                    $uan_insert
                );
            }
            ...
            if ($row["esi_no"]) {
                $esi_insert = [
                    ...
                    "type" => "esi",
                    "start_date" => $row["esi_start_date"]
                        ? date("Y-m-d", strtotime($row["esi_start_date"]))
                        : null,
                    "end_date" => $row["esi_end_date"]
                        ? date("Y-m-d", strtotime($row["esi_end_date"]))
                        : null,
                    "location" => $row["esi_area"],
                    "status" => "active",
                ];
                StaffPfEsiDetail::updateOrCreate(
                    ["staff_id" => $staff_id, "type" => "esi"],
                    $esi_insert
                );
            }
            ...
            $staff_personal_details = [
                ...
                "marriage_date" =>
                    strtolower($row["marital_status"]) == "married"
                        ? date("Y-m-d", strtotime($row["marriage_date"]))
                        : null,
